...
for b = 1:length(baseDirs)
    ...
    for k = 1:length(subFolders)
        ...
        if ~isempty(matFile)
            ...
            [avgDaytimeStateLengths, avgNighttimeStateLengths] = computeDayNightStateLengths(sleepIntervals, appropriateTtimestamps);
    ...
    results.(condition).ZTData = allZTData;
    results.(condition).BoutDurations = allBoutDurations;
    results.(condition).InterBoutIntervals = allInterBoutIntervals;
    results.(condition).AvgDaytimeStateLengths = allAvgDaytimeStateLengths;
    results.(condition).AvgNighttimeStateLengths = allAvgNighttimeStateLengths;
    ...
    results.(condition).StateOccupancy = pooledStateOccupancy;
    ...
    plotBarGraph(pooledAvgBoutDurations, 'Average Bout Duration (s)', ['Pooled Average Bout Durations - ', condition], animalName, condition);
    ...
    plotBarGraph(pooledInterBoutIntervals, 'Average Inter-Bout Interval (s)', ['Pooled Inter-Bout Intervals - ', condition], animalName, condition);
    ...
    plotDayNightComparison(pooledAvgDaytimeStateLengths, pooledAvgNighttimeStateLengths, 'Average State Length (s)', ['Pooled Average State Length by Day/Night - ', condition], animalName, condition);
end
